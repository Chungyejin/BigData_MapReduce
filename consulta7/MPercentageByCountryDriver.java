package olympics.consulta7;

import olympics.consulta6.FPercentageByCountryDriver;
...
public class MPercentageByCountryDriver extends Configured implements Tool {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int result = ToolRunner.run(new Configuration(), new MPercentageByCountryDriver(), args);
        System.exit(result);
    }
    ...
    public static class GenderMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (columns.length > 9 && !columns[9].isEmpty() && !columns[9].equals("0") && columns.length > 4 && !columns[4].isEmpty()) {
                String country = columns[9].replace("\"", "");
    ...
    public static class GenderReducer extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            if (totalCount > 0) {
                ...
                context.write(new Text(key.toString()), new Text(String.format("Male: %.2f%%", malePercentage)));
            } else {
